...
public class FileUtils {
		...
		while(iter.hasNext()) {
			...
			for (MultipartFile multipartFile : list) {
				if(multipartFile.isEmpty() == false) { //파일이 비지 않았다면
					...
					boardFile.setCreatorId("admin"); // 보드 번호 
	...
	//ComplainFiles
	public List<CPFiles> parseCPFileInfo(MultipartHttpServletRequest multipartHttpServletRequest) {

		if (ObjectUtils.isEmpty(multipartHttpServletRequest)) { // 아무런 파일이 없을 때
			return null;
		}

		List<CPFiles> fileList = new ArrayList<CPFiles>();
		...
		while (iter.hasNext()) {
			...
			for (MultipartFile multipartFile : list) {
				if (multipartFile.isEmpty() == false) { // 파일이 비지 않았다면
					...
					CPFiles complainFile = new CPFiles();
					complainFile.setCreatorId("admin"); // 보드 번호
					complainFile.setFileSize(multipartFile.getSize()); // 파일 사이즈
					complainFile.setOriginalFileName(multipartFile.getOriginalFilename());
					complainFile.setStoredFilePath(path + "/" + newFileName); // 파일 위치
					fileList.add(complainFile); // 파일리스트에 추가
